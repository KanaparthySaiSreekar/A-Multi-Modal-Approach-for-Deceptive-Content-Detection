...
class checkTitle:
    ...
    def availability_on_web(self, results):
        ...
        if not flag:
            ...
            self.required[self.most_index] = self.required[self.most_index].replace(
                "\n", ""
            )
            self.required[self.most_index] = self.required[self.most_index].lstrip()
            if (
            ...
            self.contexts[self.most_index] = self.contexts[self.most_index].replace(
                "\n", ""
            )
            self.contexts[self.most_index] = self.contexts[self.most_index].lstrip()
            if (
                calculate_sentence_similarity(
    ...
    def check_similarity3(self, context):
        client = claude_client.ClaudeClient(
            "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"
        )
        ...
        if organizations is not None and len(organizations) > 0:
            ...
            print("Error: Unable to retrieve organizations")
            return False  
        ...
        response_text = claude_obj.send_message(
            input_query, conversation_uuid=conversation_uuid
        )["completion"]
        ...
        print(response_text)
        if final_response.find("Yes") != -1:
            return True

        return False

    # def check_similarity3(self, context):
    #     token = "eQhsa3ZmZTJRt7YfQds1LIAQYzQ79bfkRDSdtYF_6vZ0E6ewgy1zTrEqU1yyOOt46fhEaA."
    #     bard = Bard(token=token)
    #     input_query = f"""
    #     i have two sentences
    #     sentence1 = "{self.headline}"
    #     sentence2 = "{context}"
    #     based on the contextual similarity, is the first statement true based on second statement?, just say yes or no, dont consider specific meaning differences and dont consider specific word meaning differences and i dont want you to retrive any information and i dont want you to retrive any information just give the answer as yes or no"""
    #     print(input_query)
    ...
    #         return True
    #     elif (
    #             final_response.lower().find("**false**") != -1
    #             or response_text.lower().find(
    ...
    #         "no"
    #     ) != -1
    #             or response_text.lower().find("**no**") != -1):
    #         return False
...
# checkTitle("hii").check_similarity3("hello")
